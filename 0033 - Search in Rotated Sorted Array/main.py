...
Por exemplo:
Lista: [5, 6, 7, 1, 2, 3, 4]
trecho da esquerda (maior): [5, 6, 7]
trecho da direita (menor): [1, 2, 3, 4]
...
def search(nums, target):
    ...
    while l <= r:
        ...
        # binary search comum: caso da lista estar ordenada de l à r
        if nums[l] < nums[r]:
        ...
        # caso a lista não esteja ordenada de l à r
        else:
            if target > nums[l]: # caso o alvo esteja na parte esquerda (maior) da lista
                if nums[l] > nums[m]: # caso haja disrupção de entre l e m da lista
                    ...
                    r -= 1
                else: #caso não haja disrupção na lista
                    if target < nums[m]:
                        r = m - 1
                    elif target > nums[m]:
                        l = m + 1
                ...
                if nums[l] < nums[m]: # caso não haja disrupção de l a m
                    l = m + 1
